...
void rotateMotor(DCMotor_t *DCMotor, uint8_t direction);//DCMotor_t구조체에 모터드라이버에 연결되는 포트와 핀을 담아 넘기고 방향을 넘기면 해당 모터가 해당 방향으로 회전
void stopMotor(DCMotor_t *DCMotor);//DCMotor_t구조체에 모터드라이버에 연결되는 포트와 핀을 담아 넘기면 해당 모터 정지
